...
create index if not exists memberships_client_idx on public.memberships(client_id); create index if not exists intake_user_idx on public.intake_requests(user_id); create index if not exists intake_client_idx on public.intake_requests(client_id); create index if not exists intake_status_idx on public.intake_requests(status);
