...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(user.email){
      ...
      const emailResult=await sendTransactionalEmail({to:user.email,subject:`Recibo Divinos · orden ${order.id.slice(0,8).toUpperCase()}`,eyebrow:'Pago confirmado',title:'Gracias por tu compra',preheader:`Tu recibo por $${Number(order.total).toFixed(2)} está listo.`,paragraphs:[order.fulfillment_type==='pickup'?'Recibimos tu pago. Te avisaremos cuando tu compra esté lista para recogido autorizado.':`Recibimos tu pago y añadimos la botella a tu colección${location?` en ${location}`:''}.`],details:[['Orden',order.id.slice(0,8).toUpperCase()],['Captura PayPal',String(payment.id)],['Total',`$${Number(order.total).toFixed(2)} USD`],['Entrega',order.fulfillment_type==='pickup'?'Recogido autorizado':'Guardar en Divinos']],actionLabel:'Ver mi cuenta Divinos',actionUrl:`${site}/#/`,finePrint:'Conserva este correo como comprobante. El cargo aparecerá procesado por PayPal.',idempotencyKey:`wine-order-${order.id}`}).catch(()=>null)
