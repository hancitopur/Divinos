...
export function ClientShop() {
  ...
  useEffect(()=>{if(!session)return;supabase.from('wine_orders').select('id,paypal_order_id,total,created_at').in('status',['created','manual_review']).not('paypal_order_id','is',null).order('created_at',{ascending:false}).limit(1).maybeSingle().then(({data})=>setPendingPayment(data))},[session])
  useEffect(()=>{
