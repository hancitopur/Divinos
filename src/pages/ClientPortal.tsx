...
export function ClientShop() {
  ...
  return <div className="stack client-page private-shop">
    ...
    {staffPreview?<div className="info"><b>Vista previa administrativa.</b> Puedes revisar toda la tienda; las compras están desactivadas en esta vista.</div>:activeMember?<div className="info"><b>Precio de miembro activado.</b> Tu cuenta recibe el precio preferente mostrado.</div>:<div className="legal-alert"><b>Precio regular.</b> Puedes comprar para recogido. Al activar una membresía recibes 15% de descuento y puedes escoger almacenamiento según el plan.</div>}
    ...
    {offers===null?<Loading/>:offers.length===0?<div className="client-success"><span>◌</span><h2>Próximamente</h2><p>Las nuevas llegadas aparecerán aquí primero para los miembros de Divinos.</p></div>:<div className="shop-grid">{offers.map((offer)=>{
      ...
      const memberPrice=Number(offer.price),regularPrice=Math.round(memberPrice*115)/100,unit=activeMember?memberPrice:regularPrice,fee=Math.round(unit*7.5)/100,total=unit+fee
      ...
      return <article className={offer.featured?'featured':''} key={offer.id}>
        ...
        <div className="shop-copy"><small>{offer.offer_source==='member'?'Colección de miembro':'Inventario Divinos'}</small><h2>{wine?.name} {wine?.vintage||''}</h2><p>{String(offer.description||wine?.producer||'').replace(/^\[DEMO\]\s*/, '')}</p>
          ...
          <div className="shop-tier-prices"><span className={activeMember?'selected':''}><small>Miembro</small><b>{money(memberPrice,lang)}</b></span><span className={!activeMember?'selected':''}><small>Regular · +15%</small><b>{money(regularPrice,lang)}</b></span></div>
