...
export function ClientShop() {
  ...
  return <div className="stack client-page private-shop">
    <div className="shop-hero"><span className="landing-kicker">Acceso anticipado para miembros</span><h1>Cava Privada</h1><p>Compra nuevas llegadas de Divinos y botellas verificadas de otros miembros. Al pagar, quedan automáticamente en tu cava.</p>{!staffPreview&&<Link className="btn secondary" to="/sell">Vender una botella de mi colección</Link>}</div>
    {message&&<div className={message.type==='ok'?'info':'error'}>{message.text}</div>}
    {staffPreview?<div className="info"><b>Vista previa administrativa.</b> Puedes revisar toda la tienda; las compras están desactivadas en esta vista.</div>:!physical&&<div className="legal-alert"><b>Tu plan Digital no incluye almacenamiento físico.</b> Puedes ver las ofertas; cambia a Reserva o Colección para comprar y guardar automáticamente.</div>}
    ...
    {offers===null?<Loading/>:offers.length===0?<div className="client-success"><span>◌</span><h2>Próximamente</h2><p>Las nuevas llegadas aparecerán aquí primero para los miembros de Divinos.</p></div>:<div className="shop-grid">{offers.map((offer)=>{
      ...
      return <article className={offer.featured?'featured':''} key={offer.id}><div className="shop-image"><Thumb path={wine?.label_photo_path} label={`${wine?.name||'Vino'} ${wine?.vintage||''}`} />{offer.featured&&<span>Selección Divinos</span>}</div><div className="shop-copy"><small>{offer.offer_source==='member'?'Colección de miembro':'Inventario Divinos'} · {[wine?.region,wine?.country].filter(Boolean).join(' · ')}</small><h2>{wine?.name} {wine?.vintage||''}</h2><p>{offer.description||wine?.producer}</p><div className="shop-price"><strong>{money(total,lang)}</strong><span>{money(offer.price,lang)} + {money(fee,lang)} servicio</span></div><div className="row between"><span className="badge">{left} disponible{left===1?'':'s'}</span><button className="btn" disabled={staffPreview||!physical||!age||left<1||busy!==null} onClick={async()=>{setBusy(offer.id);setMessage(null);const {data,error}=await supabase.functions.invoke('paypal-wine-order',{body:{offerId:offer.id,quantity:1,acceptAge:age}});if(error||data?.error){setMessage({type:'error',text:data?.error||error?.message||'No se pudo iniciar el pago.'});setBusy(null)}else window.location.href=data.approval_url}}>{staffPreview?'Vista previa':busy===offer.id?'Conectando…':'Comprar con PayPal'}</button></div></div></article>
    })}</div>}
