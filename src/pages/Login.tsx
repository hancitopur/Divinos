  ...
  return (
    <div className="auth">
      <form className={`card stack ${checkoutOffer ? 'checkout-auth-card' : ''}`} onSubmit={submit}>
        ...
        {checkoutOffer && (()=>{const wine=Array.isArray(checkoutOffer.wines)?checkoutOffer.wines[0]:checkoutOffer.wines;const price=storePrices(Number(checkoutOffer.price)).public;const name=String(wine?.name||'Vino');const vintage=String(wine?.vintage||'');const displayName=vintage&&!name.includes(vintage)?`${name} ${vintage}`:name;return <div className="checkout-auth-summary"><img src={wineProductPhoto(wine?.name)} alt={`Botella de ${wine?.name||'vino'}`}/><div><small>Tu selección</small><b>{displayName}</b><span>{[wine?.producer,wine?.region].filter(Boolean).join(' · ')}</span><strong>{money(price,lang)}</strong><em>{purchaseMode==='storage'?'Guardar en Divinos requiere una membresía física activa.':'Compra para recogido autorizado.'}</em></div></div>})()}
